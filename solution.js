...
promiseA.then( (valh) => console.log("asynchronous logging has val:",valh) );
...
const compareToTen = (num) => {
  ...
  const promise = new Promise( (res, rej) => {
    if(num >= 10){
     res(`${num} is greater than or equal to 10, resolved!`)
    } else {
    rej(`${num} is less than 10, error!`)
    }
  })
...
const order = 'Slurpy'
const drink = new Promise ((res, rej) => {

console.log(`I'll be right back with your ${order}`)

const orderCannotBeFulfilled = true

if(orderCannotBeFulfilled === true) {
   setTimeout(() => {   
   res(console.log(`Sorry we are all out of ${order}`))
    }, 2000)
} else {
   setTimeout(() => {   
   rej(console.log(`Here is your ${order}`))
    }, 4000)}

})

drink.then((result) => result).catch((err) => {err})
...
const makeAllCaps = (arr) => {
 ...
 arr.map(x => {
    ...
    } else throw new Error("Not All Items are strings");
...
const totalSales = new Promise ((rej, res) => {
...
const playerFunction = new Promise ((rej, res) => {
